...
def stash_local_changes() -> bool:
    """Temporarily preserve uncommitted work before switching branches."""

    if not run_git_command(["git", "status", "--porcelain"]).strip():
        return False
...
def restore_local_changes() -> None:
    """Restore any preserved local edits after the feature branch is ready."""

    if not run_git_command(["git", "stash", "list"]).strip():
        return
    ...
    try:
        ...
        print(
            "Warning: local changes could not be restored automatically; "
            f"they remain stashed. {exc}"
        )


def create_branch(ticket_number: str) -> str:
    """
    Create a unique ticket-specific feature branch based on the
    latest remote main branch so GitHub PRs have a valid ancestry.
    ...
    # Always branch from the latest remote default branch to avoid
    # creating a PR from a stale local branch with no shared history.
    run_git_command(
